...
describe("e2e Task Scheduler-Cell tracing-Filtro Node & Event Details Record",()=>{
    ...
    const master = new Adele
        ...
        it("Filtrar por Nodo", ()=>{
            cy.get('.ReactSelect__value-container').then(($filter) => {
                if ($filter.text().includes('Node')) {
                    cy.wait(250)
                } else {
                    cy.get('input[id="networkscope_search_type"]').type('Node',{force:true}).type('{enter}').wait(1000)
                }
              });
            cy.get('input[id="searchValue"]').type('ABASCAL',{force:true}).wait(2000).type('{enter}')
        })
